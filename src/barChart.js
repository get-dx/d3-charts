...
export class BarChart {
  ...
  renderTrendLine() {
    ...
    if (this.lr) {
      ...
      const x2 = this.x(xScaleDomain[xScaleDomain.length - 1]) + this.x.bandwidth() / 2;
  
      const y1 = Math.max(yScaleRange[1], Math.min(yScaleRange[0], this.y(this.lr.intercept)));
      const y2 = Math.max(yScaleRange[1], Math.min(yScaleRange[0], this.y(this.lr.slope * (xScaleDomain.length - 1) + this.lr.intercept)));
